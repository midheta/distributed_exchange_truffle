...
contract('Exchange Order Tests', function (accounts) {
    ...
    //     }).then(function(orderBook) {
    //         assert.equal(orderBook.length, 0, "OrderBook should have 0 sell offers");
    //     });
    // });


    it("should be created limit buy order", function () {
        var myExchangeInstance;
        return exchange.deployed().then(function (instance) {
            myExchangeInstance = instance;
            return myExchangeInstance.sellToken("FIXED", web3.utils.toWei('2', 'finney'), 5, {from: accounts[1]});
            ...
            assert.equal(orderBook[1].length, 1, "OrderBook should have 1 sell volume has one element");
            assert.equal(orderBook[1][0], 5, "OrderBook should have a volume of 5 coins someone wants to sell");
            return myExchangeInstance.buyToken("FIXED", web3.utils.toWei('3', 'finney'), 2);
        }).then(function(txResult) {
            ...
            assert.equal(orderBook[0].length, 1, "OrderBook remained 1 sell offer");
            assert.equal(orderBook[1].length, 1, "OrderBook should have 1 sell volume");
            return myExchangeInstance.getBuyOrderBook.call("FIXED");
        }).then(function(orderBook) {
            assert.equal(orderBook.length, 0, "OrderBook should have 1 buy offers");
        });
    });
